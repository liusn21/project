...
def build_byte_vocab(vocab_path):
    """Generate single-byte vocabulary file (261 tokens: 5 special + 256 hex bytes).

    Compatible with uer BertTokenizer. Each byte value (00-ff) is a token.
    Use with --raw_token_type byte in data generation scripts.
    """
    with open(vocab_path, 'w') as f:
        ...
        for i in range(256):
            f.write(f'{i:02x}\n')
    print(f"Generated byte vocab ({256 + 5} tokens) -> {vocab_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, required=True,
                        choices=['bigram', 'byte'],
                        help='bigram: build from wordpiece.tokenizer.json; byte: generate 256 single-byte vocab')
    parser.add_argument('--output', type=str, required=True,
                        help='Output vocabulary file path')
    args = parser.parse_args()

    if args.mode == 'bigram':
        build_vocab(args.output)
    elif args.mode == 'byte':
        build_byte_vocab(args.output)
